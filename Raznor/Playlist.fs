...
module Playlist =
  ...
  let update (msg : Msg) (state : State) : State * Cmd<Msg> * ExternalMsg option =
    ...
    | SetLoopState loopState ->
        { state with loopState = loopState }, Cmd.none, None
    | PlaySong song ->
    ...
    ListBox.create
      [ ListBox.dataItems songs
        ...
        ListBox.itemTemplate
          (DataTemplateView<Types.SongRecord>
            .create(fun item -> songTemplate item dispatch)) ]
    ...
    StackPanel.create
      [ StackPanel.spacing 8.0
        StackPanel.onPointerPressed (fun args ->
          ...
          printfn "%A" point)
        StackPanel.children
          [ TextBlock.create [ TextBlock.text "Nothing to play here :)" ] ] ]
